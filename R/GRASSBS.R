...
#''calc_basin_stats' and reads data to data.frame. For each UID, the corresponding
#'specified statistic is returned as a data.frame.
#'
#'@param basin_df A data frame with x-y coordinates --labeled as x & y-- of pour points to delineate and a unique identifier feild --labeld UID--.
#'@param stat_dir Path to output directory with r.univar statistic txt files from 'calc_basin_stats' as character vector.
#'@param stat Statistic to return, one of n, min, mean, max, MAE, stddev, var or sum as string.
#'@return Two column data.frame with UID field and corresponding upstream statistic --stat--.
#'@export
...
#'are calculated for a input raster layer and returned as a data.frame with the UID and specified
#'statistic. The GRASS r.water.outlet and r.univar functions are used for computation, therefore,
#'an initialized GRASS environment with a digital elevation model must be active, as well as the
#'layers for which statistics are derived. Note these are wrapper functions that assume that the
#'GNU parallel package is available.
#'
...
#'@return Two column data.frame with UID field and corresponding upstream statistic --stat--.
#'@export
get_basin_stats<-function(basin_df,procs,stat_rast,stat)
{
  ...
  stat_vec<-stats_to_df(basin_df,stat_dir,stat)
